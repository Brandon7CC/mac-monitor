//
//  CustomizableSystemProcessExecTableController.swift
//  ProjectSutro
//
//  NSTableView controller for process execution events (Sonoma+ / macOS 14).
//  5 columns: Timestamp (hidden by default), Process name, Signing ID, Process path, Command line
//
...
class CustomizableSystemProcessExecTableController: SystemEventsTableController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupColumns(for: .processExec(customizable: true))
...
@available(macOS 14.0, *)
extension CustomizableSystemProcessExecTableController {
    func tableView(_ tableView: NSTableView, heightOfRow row: Int) -> CGFloat {
        ...
        guard let message = messageForRow(row),
              ...
              !commandLine.isEmpty,
              let commandLineColumn = tableView.tableColumn(withIdentifier: MessageTableColumnID.commandLine.identifier),
              !commandLineColumn.isHidden else {
            return minRowHeight
        }
...
@available(macOS 14.0, *)
extension CustomizableSystemProcessExecTableController: NSMenuDelegate {
